...
export function ConversationsChart({ series, loading, range, onRangeChange }: ConversationsChartProps) {
  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-slate-800 bg-card">
      <header className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-secondary border-border/60 p-1">
          {[7, 30, 90].map((r) => (
            <button
              key={r}
              type="button"
      ...
      <footer className="flex items-center gap-4 border-t border-slate-800 px-5 py-3 text-xs text-muted-foreground">
